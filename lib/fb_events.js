...
function sortDeterminer(inputVenue) {
  ...
  else if (inputVenue.toLowerCase().indexOf('brookland') !== -1) {
    return ['brookland', 1000];
  } else if (inputVenue.toLowerCase().indexOf('music farm columbia') !== -1) {
    return ['musicfarmcolumbia', 2000];
  } else if (inputVenue.toLowerCase().indexOf('tin roof columbia') !== -1) {
    return ['tinroofcolumbia', 3000];
  } // eslint-disable-line brace-style

  else if (inputVenue.toLowerCase().indexOf('orange peel') !== -1) {
    return ['orangepeel', 1000];
  } else if (inputVenue.toLowerCase().indexOf('grey eagle') !== -1) {
    return ['greyeagle', 2000];
  } else if (inputVenue.toLowerCase().indexOf('asheville music hall') !== -1) {
    return ['avlmusichall', 3000];
  } else if (inputVenue.toLowerCase().indexOf('asheville civic center') !== -1) {
    return ['civiccenter', 4000];
  }
  return ['other', 5000];
}

function getFacebookData(batchArray) {
