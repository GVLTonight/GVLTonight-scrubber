...
function sortDeterminer(inputVenue) {
  ...
  } else if (check(inputVenue, 'tipsy')) {
    return ['tipsymusicpub', 1100];
  ...
  // charlotte nc
  else if (check(inputVenue, 'charlotte')) {
    return ['charlotte', 1000];
  }
  return ['other', 5000];
}
    ...
    getKey('fb').then((key) => {
      ...
      FB.api('', 'post', {
        ...
        for (let i = 0; i < response.length; i++) {
          ...
          // IF one of the response object's body.error exists
          // -- stop everything, just crash.
          if (JSON.parse(response[i].body).error !== undefined) {
            log[market].error = JSON.parse(response[i].body).error;
            writeLog(file, log, true).then(() => {
              reject(console.log(JSON.parse(response[i].body).error)); process.exit(0);
            });
          }
          // --
